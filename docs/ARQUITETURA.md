...
- **O recorte das leituras de relatório é LISTA, nunca nulo (F60).** Toda RPC `rel_*` que recorta por filial recebe `p_filiais smallint[]` e liga o recorte a uma coluna de filial por `= any (p_filiais)`; NULL e `'{}'` devolvem ZERO linhas. O **consolidado** (`geral`) é a lista EXPLÍCITA de TODAS as filiais, **inclusive as desativadas**, montada num lugar só — `filiaisDoConsolidado`/`recorteDeFiliais` em `src/lib/queries/relatorios/recorte-filiais.ts`, o único ponto onde o `filialId: number | null` da camada vira lista (a camada segue `number | null` por dentro até a F63). ⚠ Nunca `listarFiliais()` para montar consolidado: ela lê só as ativas e apagaria em silêncio o acervo de filial desativada. `rel_*` nova passa pela trava — `src/lib/validators/rpcs-recorte-sql.test.ts` na mesa e o bloco 7 de `supabase/tests/catalogo_secdef.sql` no CI; a única exceção (`rel_saldo_colaborador`, que recorta por pessoa) mora em `k_excecoes_recorte`, lá. A régua inteira: emenda F60 da [`MATRIZ-REGRAS.md`](MATRIZ-REGRAS.md) (R-ACC-73 em diante). As migrations `0141`–`0145` estão aplicadas no ensaio e em produção desde 17/09/2026 (atas F60 (l) e (m) em [`DECISOES.md`](DECISOES.md)); a troca de assinatura seguiu a receita "A janela do `drop`" do [`RUNBOOK-BANCO.md`](RUNBOOK-BANCO.md), que vale de novo para qualquer função chamada pelo app que mude de nome.
- **A porta não aceita nulo nas `rel_*`.** Nenhuma `rel_*` está em `ARGUMENTOS_ANULAVEIS` (`src/lib/supabase/rpc.ts`): `chamarRpc(client, 'rel_…_filiais', { p_filiais: null, … })` não compila (`linhas-tipos.test.ts`). O único nulo de domínio que sobrou num recorte de filial é o `ALCANCE_DO_RESET` da Zona destrutiva — escrita, não leitura, e da F67.
- **`/itens` e o saldo de itens em DOIS NÍVEIS, numa leitura paginada.** `rel_saldo_itens_filiais(p_filiais, p_ate)` devolve uma linha por (filial do recorte, item) e o NÍVEL DO TOTAL (`filial_id` NULL), calculado sobre o CONJUNTO — nunca a soma das linhas, porque os clamps por chamado não são aditivos entre filiais. A ÚNICA leitura dela em `src/**` é `lerSaldoItensEmNiveis` (`src/lib/queries/relatorios/itens.ts`, por `paginarTodos` com a ordem do corpo imposta — trava por AST em `relatorios/itens.test.ts`): a resposta tem (filiais + 1) × itens linhas, e o `max-rows` de 1.000 do PostgREST a cortaria calado. No volume de hoje são 2 idas por render de `/itens`, em vez das 7 da F59. `/itens` passa todas as filiais: as colunas são as linhas das ATIVAS, o consolidado é o nível do total, e a diferença é `estoqueForaDasColunas` (`src/lib/queries/itens.ts`: `separarNiveisDeSaldo`, `montarSaldosPorFilial`, `somarSaldosDaSelecao`). ⚠ Quem ler a função em SQL escolhe o nível pelo nulo (`where filial_id is null` para o total) — sem isso `select … into` pega uma linha qualquer e `count(*)` conta o dobro.
- **O as-of calcula a filial NA DATA, e o custo dele está orçado.** `rel_estoque_asof_filiais` ancora em `ativos` e acha a última movimentação efetiva por `cross join lateral`; o recorte vale sobre a filial CALCULADA, nunca sobre `ativos.filial_id` (pré-filtrar pela filial de hoje tira do passado o ativo transferido depois). Mudar o corpo dele deixa `src/lib/validators/asof-orcamento.test.ts` vermelho até `docs/perf/asof-orcamento.json` ser medido de novo.
- **As três tabelas do período têm teto com aviso.** `TETO_LINHAS_TABELA` (2.000, `src/lib/relatorios/teto-tabela.ts`): acima dele a tabela mostra as mais recentes e o total EXATO (um `count` a mais), e o snapshot guarda o corte na chave opcional `tabelasTruncadas` da forma V2 — nunca `meta.schema: 3`.
...
- **Migrations** em `supabase/migrations/` são a **fonte da verdade** desde a F1 (`0001`→`0145`; a `0029` não existe). Nunca editar uma migration já aplicada — toda mudança é uma nova. O rascunho original `schema.sql` foi aposentado (21/07/2026). *(Emenda F19: o intervalo estava congelado em `0040`. Emenda de 30/08/2026: estava congelado em `0057`. Emenda F59, 16/09/2026: estava em `0124`. Emenda F60, 17/09/2026: estava em `0140` — e as `0141`–`0145` estão no repositório sem apply em banco real, ver o Anexo A do [`RUNBOOK-BANCO.md`](RUNBOOK-BANCO.md).)*
...
| **ler uma tabela INTEIRA em lote** (backup, snapshot, relatório) | `paginarTodos(rotulo, pagina, cap)` ou `paginarPorIds(rotulo, ids, pagina, cap)` de `src/lib/queries/relatorios/comum.ts`. O `cap` é OBRIGATÓRIO (esquecer é erro de `tsc`) e é a constante `CAP_*` do DOMÍNIO lido, com a conta escrita ao lado (≥ 20× o volume de 16/09/2026, piso 10.000); acima dele a leitura LANÇA. Keyset (`{ porChave, chaveDe }`) só quando o cursor é a PK `id` de uma tabela — `comum.test.ts` trava por AST; ordem composta, RPC e lista de tela ficam em OFFSET, com o motivo na chamada. Domínio que passou do teto refaz a conta, nunca troca a constante por uma maior (F60) |
| **uma RPC de relatório nova (`rel_*`)** | o recorte como LISTA: `p_filiais smallint[]` ligado por `<tabela>.filial_id = any (p_filiais)` em conjunção direta, `language sql stable security invoker set search_path = public`, NÃO `strict`, e `revoke all … from public, anon` + `grant execute … to authenticated, service_role` na mesma migration. `rpcs-recorte-sql.test.ts` diz na mesa o que falta; o bloco 7 de `catalogo_secdef.sql` confere no CI. Trocar a assinatura de uma que o app já chama é NOME NOVO + `drop` da velha em arquivo separado, depois do deploy (a receita "A janela do `drop`" do `RUNBOOK-BANCO.md`). Se ela alcançar o visualizador (módulo que aceita `DbClient`), a catraca de `fronteira-viewer.test.ts` sobe — com ata (F60) |
| **não pagar duas vezes a mesma leitura no mesmo request** | `memoizarPorUnidades` (`src/lib/queries/memo-do-request.ts`) com um `cache(() => new Map())` no escopo do módulo e a chave primitiva `chaveDasUnidades` (`src/lib/auth/recorte-leitura.ts`) — `cache(fn)` com objeto de unidades como argumento NUNCA acerta (o React compara por `Object.is`). Só para contagem de acervo recortada; nunca vínculo nem cargo (ADR-002 §4) (F60) |
...
| **um argumento de RPC que aceita `null`, ou um retorno que o gerador diz não-nulo e vem nulo** | os mapas nominais de `src/lib/supabase/rpc.ts` — `ARGUMENTOS_ANULAVEIS`, `COLUNAS_DE_RETORNO_ANULAVEIS`, `ESCALARES_ANULAVEIS` —, cada entrada com o MOTIVO e a EVIDÊNCIA (trecho do corpo vivo), conferida por `rpc-mapas-sql.test.ts`. *(Emenda F60: `RECORTE_DO_RELATORIO` deixou de existir — as `rel_*` recebem `p_filiais smallint[]` e NÃO entram neste mapa; uma `rel_*` que voltar a ele estaria declarando um nulo que significa "tudo", e a trava de recorte a reprova.)* |
...
| o schema do banco | **nova** migration em `supabase/migrations/` + `npm run db:types` **apontado para PRODUÇÃO** (`DB_TYPES_PROJECT_REF=<ref de prod>`) — a CLI **não** está linkada, e gerar do ensaio apaga do arquivo os objetos que só produção tem (F41). *(Emenda F60: quando o arquivo entra com hand-fix do estado FINAL da cadeia antes do apply — o caso da F60, cujo `drop` só vai a produção depois do deploy —, ele se confere DUAS vezes: regenerando do ENSAIO depois do apply da cadeia inteira, com o diff só para a evidência e nunca para o commit; e regenerando de PRODUÇÃO depois do `drop`, que é o arquivo que substitui o hand-fix. Regenerar de produção antes do `drop` devolveria ao tipo as assinaturas velhas. Anexo A do [`RUNBOOK-BANCO.md`](RUNBOOK-BANCO.md), entrada `0141`–`0145`.)* |
...
| **como um número de item é MOSTRADO** | `src/lib/itens/lista.ts` — `emUsoDoSaldo` (a coluna "Em uso", derivada das quatro que `rel_saldo_itens_filiais` devolve — o nível do total ou a linha da filial, §8), `saldoDoRecorte` (o recorte de filial) e `montarLinhasDeItem`. A tela, a linha expansível e o CSV leem as **mesmas** funções: divergir deixou de ser possível |
...
| uma rota nova no grupo `(app)` | a rota **e** três lugares que a tornam achável, no MESMO commit: `sidebar-nav.tsx`, `paleta-comandos.tsx` e `src/lib/ajuda/conteudo/mapa-das-telas.ts`. Mais a matriz `COBERTURA` de `src/lib/ajuda/registry.test.ts`, que derruba o `npm run test` se a rota não apontar para uma página de ajuda. *(F60:)* E `export const maxDuration = 60` na `page.tsx`, com o motivo — 300 só se a página hospedar Server Action que cresce com o acervo inteiro (hoje `admin/importar` e `dev/destrutivo`), porque o teto da página vale também para as Server Actions dela |

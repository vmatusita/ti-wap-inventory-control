...
| `scripts/perf/equivalencia-rel.mjs` — **versionado** (lote 2 + os modos `*-real` da revisão final + o modo `mesmo-nome` e a guarda fechada da F66) | 89.308 | `7e6d85a9cce24cc73f62399e970ca34c98a9e469140c9bf1d5cb9767fdde7902` |
...
**F66, revisão adversarial (24/09/2026) — a segunda diferença declarada.** A guarda `validarBloco` ficou FECHADA: a lista
de funções (`FUNCOES_PERMITIDAS`, lida pelo léxico da trava de mesa, dentro dos literais e dos `$tag$` também), a de
configurações com o valor de cada uma, dois `deallocate` para CADA `prepare`, e `--ref` conferido contra o
`scripts/env-guard.ts` em todo modo `gerar-*`. A versão anterior (o modo `mesmo-nome`) tinha 83.951 bytes e sha256 (LF)
`87bc862a052b0130bf1f7c8828218c2c538357c2848e929437f80708b39a6a9f`. **O SQL emitido não mudou:** os 64 blocos de
`gerar-equivalencia`, `gerar-custo`, `gerar-equivalencia-real`, `gerar-custo-real` e `gerar-mesmo-nome` (antes e depois),
ensaio e produção, saíram idênticos aos da versão anterior (`diff -r` vazio) — e todos passam pela guarda nova.

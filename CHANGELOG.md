...
- 🚧 **`force row level security` ganhou o par executável, 40 dias depois de virar regra.** A `0070` o proíbe desde 30/07/2026, com as duas razões escritas uma quatro linhas abaixo da outra — e **nenhuma asserção conferia se a proibição estava sendo cumprida**. Agora confere, nas 21 tabelas, e não só nas quatro que a migration cita. É **R-ACC-29** em [`docs/MATRIZ-REGRAS.md`](docs/MATRIZ-REGRAS.md), junto de mais seis regras novas de enumeração (R-ACC-30 a R-ACC-35), cada uma com o número medido no CI ao lado.

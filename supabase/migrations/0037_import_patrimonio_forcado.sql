-- Migration 0037 — import de startup: patrimônio FORÇADO fora do formato canônico
-- (OS-F7J / decisão do Johnny 20/07/2026).
--
-- CONTEXTO. A régua do patrimônio exigia o formato canônico `^[A-Z]{2,4}\d{7}$` (ou nulo).
-- Valores reais que fogem disso (`LEA7LYHQH4`, `STF003LOC` — letras no meio) eram sempre
-- BLOQUEADOS. O Johnny (20/07/2026) decidiu: o operador pode FORÇAR um valor fora do padrão
-- como patrimônio ("usar mesmo assim" no preview do import) — o ativo passa a ser
-- identificado por esse número, mesmo não-canônico (a busca casa por substring, já era
-- ilike). O motor (src/lib/import/plano.ts, op `forcar_patrimonio`) e a UI decidem QUANDO
-- forçar; o banco só precisa parar de rejeitar o valor.
--
-- COMO. `create or replace` PURO (4 args, SEM drop), como a 0034/0035/0036: assinatura
-- IDÊNTICA (jsonb, text, jsonb, jsonb) → sem overload, pg_proc segue 1 linha. O corpo é o
-- da 0036 VERBATIM, com EXATAMENTE uma mudança (marcada "F7J" no passo 1d): a validação do
-- patrimônio deixa de exigir o formato canônico e passa a checar só a SANIDADE (não-nulo →
-- não-vazio e ≤ 60 caracteres). A régua de FORMATO é agora do motor/UI (defesa em
-- profundidade movida para lá, onde o "forçar" é uma escolha explícita do operador). O
-- null segue válido (importa com pendência). Passo 4a insere `e->>'patrimonio'` cru (já
-- inseria), as conferências 5b/5c comparam por string exata (funcionam com não-canônico),
-- o índice único e a unicidade 1e dedupem por string — tudo já compatível. Qualquer outra
-- diferença vs. a 0036 é BUG. Grants reafirmados no fim (defesa em profundidade).
--
-- NÃO TOCA DADO. Só afeta imports FUTUROS (troca o corpo da função). Nenhum UPDATE.
--
-- Aditiva. Aplicar no DEV; o orquestrador entrega o SQL para o Johnny rodar em produção
-- pelo SQL Editor (o gate do classificador barra DDL com `delete from ativos/movimentacoes`
-- em produção, como na 0033/0034/0035/0036).
...
-- Prova a F7J: um patrimônio FORA DO FORMATO canônico (`LEA7LYHQH4`) é ACEITO pela RPC
-- (o motor/UI já decidiram "forçar"). Antes a validação do passo 1d o rejeitava.
-- begin;…;rollback; não persiste nada. Dados 100% fictícios — CLAUDE.md.
--   #1 WAP0001234  — canônico normal (controle).
--   #2 LEA7LYHQH4  — NÃO-canônico forçado → entra como patrimônio cru.
-- Esperado: ativos_criados=2; #2 com patrimonio='LEA7LYHQH4'.
--
-- begin;
--   do $smoke$
--   declare v_prof uuid; v_filial smallint; v_res jsonb;
--   begin
--     select id into v_prof from public.profiles order by created_at limit 1;
--     if v_prof is null then raise exception 'Smoke: nenhum profile no DEV.'; end if;
--     perform set_config('request.jwt.claims', json_build_object('sub', v_prof)::text, true);
--     insert into public.filiais (slug, nome, ativo)
--     values ('zz-smoke-f7j', 'Smoke F7J', true) returning id into v_filial;
--     v_res := public.importar_ativos_substituir(
--       jsonb_build_object('filialId', v_filial, 'arquivoHash','deadbeefcafe', 'totalLinhasDados', 2,
--         'ativos', jsonb_build_array(
--           jsonb_build_object('patrimonio','WAP0001234','patrimonioOriginal','1234','serviceTag',null,
--             'categoria','notebook','marca','Dell','modelo','Latitude','fornecedor','WAP',
--             'memoria',null,'armazenamento',null,'processador',null,'hostname',null,'observacoes',null,
--             'dataEntrada',null,'dataAjuste',null,'estadoAlvo','em_estoque','colaborador',null,'setor',null,'chamado',null),
--           jsonb_build_object('patrimonio','LEA7LYHQH4','patrimonioOriginal','LEA7LYHQH4','serviceTag',null,
--             'categoria','celular','marca','Samsung','modelo','A54','fornecedor','WAP',
--             'memoria',null,'armazenamento',null,'processador',null,'hostname',null,'observacoes',null,
--             'dataEntrada',null,'dataAjuste',null,'estadoAlvo','em_estoque','colaborador',null,'setor',null,'chamado',null)
--         )),
--       'backups-import/smoke/plano-smoke-f7j.json',
--       jsonb_build_object('ativos',0,'movimentacoes',0,'anotacoes',0,'termos',0));
--     raise notice 'RESULTADO: %', v_res;
--     raise notice 'ATIVOS (esperado #2 patrimonio=LEA7LYHQH4): %', (
--       select jsonb_agg(jsonb_build_object('patrimonio', patrimonio, 'pendencia', pendencia) order by patrimonio)

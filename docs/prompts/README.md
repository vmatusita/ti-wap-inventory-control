...
| F6B | `F6B-melhorias-ux.md` | Melhorias de UX/features: loading, semana default dom–sáb, quebra no resumo, obs no snapshot, seção de itens nas saídas/entradas, confirmar assinatura do termo, corrigir patrimônio, sessões 24h, manual interno — **rodar depois da F6A**. **Para executar, use `F6B-ultracode.md`** — orquestração em 2 ondas (W1 semana+resumo ∥ W2 obs+tabela de itens ∥ W3 termo+patrimônio ∥ W4 sessões → integração → B1 loading ∥ B9 ajuda), com gate de entrada exigindo a F6A concluída |

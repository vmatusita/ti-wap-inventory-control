...
- [ ] **F7 — import de startup por CSV no admin** (`docs/prompts/F7-import-csv-ativos.md` + **`F7-ultracode.md`** para executar, planejada em 16/07/2026): tela `admin/importar` para o **go-live novo de cada filial** — só modo Substituir tudo (delete físico assumido, com backup automático + confirmação pelo nome da filial), preview tudo-ou-nada com erros linha a linha, **entrada com a data real do CSV** (sem data válida = marcada fora dos relatórios do período). Modo Atualizar adiado (correções são no sistema, linha por linha). **Revoga a regra "não existe importação, nunca"** (spec §10/CLAUDE.md) — decisão consciente do Johnny; a OS emenda os documentos. Roda após a F6B

...
- **Relatórios de fase** — `RELATORIO-F11.md` → `RELATORIO-F47.md` (mais `F19-RELATORIO.md` e `RELATORIO-CORRECAO-TRUNCAMENTO-1000.md`): o que cada ordem entregou, com as evidências. As pastas `f45-evidencias/`, `f46-evidencias/` e `f47-evidencias/` guardam as saídas reais das sabotagens que provam que cada trava sabe ficar vermelha — leia-as junto com o relatório da fase, nunca no lugar dele.

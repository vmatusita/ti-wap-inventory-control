...
- [ ] **F6B — melhorias de UX** (`docs/prompts/F6B-melhorias-ux.md`, roda após a F6A): loading (barra + skeletons), relatório default semana dom–sáb, quebra de linha no resumo, obs no snapshot, seção de itens nas saídas/entradas, confirmar assinatura do termo, corrigir patrimônio com rastro, sessões 24h, página de documentação interna
...
**F6A — correções pós-go-live** (planejada com o Johnny em 16/07/2026, decisões registradas na própria OS). O sistema está em produção, mas há correções de confiança pendentes: as entradas da carga inicial aparecem como movimentação do período no filtro semanal, a semântica de estoque dos itens está errada (atrelar/liberar não descontam) e as pendências aparecem para o visualizador por senha. Abra o Claude Code e cole `docs/prompts/F6A-correcoes-pos-golive.md` — **não depende de nenhum insumo do Johnny**. Em seguida, `docs/prompts/F6B-melhorias-ux.md` (loading, semana default, obs no snapshot, confirmar assinatura do termo, corrigir patrimônio, sessões 24h, manual interno). Por último, quando o export da planilha de gestão online existir, `docs/prompts/F6C-carga-saldos-itens.md` (decisão do Johnny: melhorias primeiro, cargas depois).

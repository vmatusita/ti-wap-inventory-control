...
| F7 | `F7-import-csv-ativos.md` | **Import de startup por CSV como tela do admin** (só Substituir tudo — go-live por filial; modo Atualizar adiado) — **revoga a regra "não existe importação" da spec §10** por decisão do Johnny (16/07/2026); alinhamento coluna a coluna fechado em sessão; entrada com a data real do CSV, sem data = fora dos relatórios do período. **Para executar, use `F7-ultracode.md`** (W1 motor ∥ W2 banco → W3 tela → W4 revisão adversarial). Roda após a F6B |

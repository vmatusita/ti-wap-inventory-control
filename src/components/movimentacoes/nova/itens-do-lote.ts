...
import { checklistPodeLancar, type LoteParaChecklist } from '@/lib/itens/checklist-lote'

// A regra do LOTE HOMOGÊNEO mora em `lib/itens/checklist-lote.ts` desde a revisão de
// 29/08/2026 — o SERVIDOR a reusa (`termos/preparo.ts`, dentro de `prepararTermo`), e
// módulo de `lib/` não pode depender de valor vindo de dentro do wizard: bastaria um
// `'use client'` neste arquivo para o import virar referência e a função chegar lá
// como `undefined`, com o build verde. Reexportada aqui para que o wizard e o teste
// desta pasta continuem importando do mesmo lugar de sempre.
export {
  checklistPodeLancar,
  MSG_LOTE_MISTO_SEM_LANCAMENTO,
  type LoteParaChecklist,
} from '@/lib/itens/checklist-lote'

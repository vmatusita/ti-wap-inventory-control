...
2. Pré-requisitos (se falhar, PARE): F0 mergeada na `main` (login funciona); `supabase link` configurado para o projeto dev; migration `0001_profiles.sql` aplicada; existe ao menos 1 usuário admin no projeto dev.
...
1. Converta `supabase/schema.sql` em migrations separadas e numeradas em `supabase/migrations/` (a 0001 de profiles já existe — não a recrie; remova do script o que ela já cobre):
   ...
   - `0003_tabelas.sql` — `filiais`, `motivos`, `ativos`, `movimentacoes` + TODOS os índices (inclusive o único composto `(patrimonio, coalesce(service_tag,''))` — regra do patrimônio duplicado).
   - `0004_maquina_estados.sql` — `status_apos_movimentacao()` + `aplicar_movimentacao()` + trigger.
   - `0005_rls.sql` — `is_admin()` + enable RLS + policies (cuidado: as de `profiles` já existem na 0001; não duplique).
...
4. Volumes e proporções (use faker pt_BR para nomes; patrimônios fictícios no formato canônico com prefixos `WAP`/`PRO`/`LEA`/`TEC` e faixa numérica 8000–9999 — inexistente nas planilhas reais):
   ...
   - **Importante:** gere as movimentações respeitando a máquina de estados — mantenha o estado corrente de cada ativo no script e só sorteie movimentação válida para ele (inserção em ordem cronológica; o trigger valida). `criado_por` = id do admin dev (busque o primeiro profile admin).
...
- [ ] Logado como viewer (curl ou SQL com anon+JWT de viewer): select em `ativos` funciona, insert falha por RLS.

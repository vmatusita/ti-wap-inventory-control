-- ============================================================================
-- HANDOFF COMPLETO E AUTOCONTIDO — migration 0040 (hardening das RPCs)
-- Rodar TUDO isto no SQL Editor de PRODUÇÃO (pbtjcalbmepmrqzprusb).
-- (O arquivo f1-hardening-rpcs-producao.sql anterior era só um ponteiro — este tem o DDL.)
-- NÃO toca dado (só recria 2 funções por create or replace). PASSO 1 = o DDL abaixo;
-- PASSO 2 = as queries de verificação no fim (esperado: 2 funções, guardas presentes).
-- ============================================================================
...
-- ============================================================================
-- PASSO 2 — VERIFICAÇÃO (rode depois do DDL acima):
-- ============================================================================
select
  p.oid::regprocedure::text as assinatura,
  position('p_contagens is null or jsonb_typeof(p_contagens)' in pg_get_functiondef(p.oid)) > 0 as guarda_contagens_ok,
  position('coalesce(auth.uid(), p_criado_por)' in pg_get_functiondef(p.oid)) > 0 as auth_uid_na_compra_ok
from pg_proc p join pg_namespace n on n.oid = p.pronamespace
where n.nspname = 'public' and p.proname in ('importar_ativos_substituir','criar_compra_lote')
order by 1;
-- ESPERADO: 2 linhas —
--   criar_compra_lote(jsonb,uuid)                         → auth_uid_na_compra_ok = t
--   importar_ativos_substituir(jsonb,text,jsonb,jsonb)    → guarda_contagens_ok   = t

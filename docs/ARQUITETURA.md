...
| a chave de versão do snapshot semanal | `chaveVersao` em `src/lib/relatorios/versao-snapshot.ts`, travada contra a `unique` da `0010` e o índice da `0013` por `chave-versao-sql.test.ts`. ⚠ `ehViolacaoDeVersao` casa pelo NOME do índice — quem recriar o índice (F65) muda os dois juntos |

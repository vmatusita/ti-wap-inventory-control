...
O CI (`.github/workflows/ci.yml`) roda em todo push na `main` e em qualquer PR, com **dois** jobs: `verificar` (`lint` → `test` → `contraste` → `build` → `verificar:actions`) e `banco-sem-docker`, que sobe um Postgres 17 pelo `services` do runner, aplica **todas as migrations** em ordem por `psql` e roda os roteiros de `supabase/tests/` pelo mesmo script que `npm run db:test` chama. Desde a F45 o cancelamento de execução em andamento vale **só em PR**: em push na `main` cada commit tem o CI dele do começo ao fim. O job de banco saiu do **Docker do Supabase** na F46 (3m52s → 57s) e o job `banco` antigo foi removido na **v1.51.1**, depois de os dois chegarem ao mesmo veredito em cinco runs seguidos — o que o `supabase start` dava de graça está declarado em [`supabase/ci/`](supabase/ci/), e a receita para rodar o mesmo banco na mesa está em [`docs/RUNBOOK-BANCO.md`](docs/RUNBOOK-BANCO.md). Desde a **F47** o job de banco faz mais duas coisas: um **gate de deriva** compara o catálogo do Postgres com `src/lib/types/database.ts` e reprova quando o banco tem tabela, coluna ou função que o arquivo não conhece; e o **injetor de mutações** (`npm run db:test:mutations`) quebra o banco de propósito, 28 vezes, exigindo que o roteiro correspondente acuse **o cenário nomeado** — é a prova de que os roteiros sabem ficar vermelhos.

> **O portão da `main` está LIGADO desde 05/09/2026 (F45)** — decisão do Johnny, ata em [`docs/DECISOES.md`](docs/DECISOES.md). `verificar` e `banco-sem-docker` são *required status checks* (o segundo era `banco` até a v1.51.1), com *require pull request* (0 aprovações) e **bypass na conta do Johnny** (`enforce_admins: false`). Provado de ponta a ponta: um PR com um roteiro SQL deliberadamente vermelho teve o merge recusado com *"the base branch policy prohibits the merge"* — [`docs/RELATORIO-F45.md`](docs/RELATORIO-F45.md) §8.5.
>
...
**Versão no ar: `1.52.0`.** A fonte única é `src/lib/versoes/registry.ts` — `VERSOES[0]` *é* a versão publicada, e um teste trava a divergência com o `package.json`. O operador vê o mesmo histórico em `/versoes`.

Entregue da **F0 à F47**: operação completa de ativos e movimentações, itens por quantidade, relatórios ao vivo e snapshots semanais com acesso por senha, termos gerados em `.docx`, import de startup por filial, cargos e permissões no Postgres, área `/dev` com zona destrutiva, e o sistema de design com `/ativos` e as três telas de `/itens` dentro do casco — a lista de itens mostrando, na própria linha, quanto tem em cada filial, e dizendo por extenso de qual filial são os números quando há filtro.

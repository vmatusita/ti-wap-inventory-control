...
- **A forma-alvo especificada, para o executor da F62/F66 não ter de inventar.** Com a decisão 6 (cargo por empresa), todo predicado que dependa de cargo vira **função sem parâmetro que devolve o CONJUNTO em que o chamador tem aquela capacidade**:
  ...
  *(Nota F59, 16/09/2026: o tipo das três primeiras é **`setof uuid`**, não `uuid[]` — consumido por `= any (array (select …))`, `uuid[]` ERRA no conjunto vazio e no NULL. A forma-alvo inteira, com a de pares, está na emenda F59 da `MATRIZ-REGRAS.md`, R-ACC-67 e R-ACC-68.)*
...
- Cada policy ganha, **em conjunção**: `and empresa_id = any (array (select public.empresas_do_membro()))`. Forma içada, uma avaliação por statement. **Nunca `e_membro(empresa_id)`** — `policies-initplan.test.ts` (F59) já barra. As policies de escrita passam a consumir `unidades_de_escrita()` na mesma forma. *(Nota F59: `unidades_de_escrita()` devolve PARES, que não cabem em `= any (array …)` de uma coluna — a forma é `(empresa_id, filial_id) in (select u.empresa_id, u.filial_id from public.unidades_de_escrita() u)`, R-ACC-68.)*

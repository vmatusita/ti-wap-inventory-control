...
//
//   1. "mostro o CTA desta ficha?"  → `podeEscreverNoEscopo`
...
import { eAdmin, escopoDeEscrita, podeEscrever } from '@/lib/auth/papeis'
...
//
// O ramo do admin vem ANTES da lista: `escopoEscrita` de um admin é a lista das
...
// não ajuda ninguém.
export function podeEscreverNoEscopo(
  ...
  return p.escopoEscrita.includes(filialId)
}
...
//
// Delega o predicado a `escopoDeEscrita` (papeis.ts) para não existir uma

...
>
> **Estado (23/07/2026):** as **três ondas do §5 foram executadas** — Onda 1 pela **F9**, Onda 2 pela **F10** e Onda 3 pela **F11** — e a **F12** fechou os dois itens que ainda pertenciam à F5 (**M12** kits e **I5** estoque mínimo); os textos abaixo descrevem o problema *como era antes* da correção e ficam como registro do diagnóstico. Continuam abertos só a carga da F6C e os itens que exigem decisão do Johnny (A8, T11; o T12 fechou na F19-UX). Nenhuma sugestão exige dependência fora da stack fechada (as duas únicas ressalvas estão marcadas), nenhuma cria custo, nenhuma mexe no modelo de acesso de nível único e nenhuma reabre decisões registradas (modo *Atualizar* do import, roles, upload de PDF assinado) — o **dark mode** era uma delas até 24/07/2026, quando a F19-UX o ligou por decisão do Johnny.
...
**~~T12 · Remover `next-themes` (dependência morta)~~ ✅ RESOLVIDO AO CONTRÁRIO em 24/07/2026 (F19-UX)**
O diagnóstico original dizia: "`package.json` lista `next-themes ^0.4.6`, mas DECISOES.md:289 fixa
'tema claro por design, toggle não adicionado' e não há `ThemeProvider`/`useTheme` em uso no `src`".
A decisão do Johnny foi a oposta de remover: **ligar o modo escuro**. A premissa da decisão de 21/07
estava errada — ela dizia que o toggle "exigiria `next-themes` (fora da stack fechada)", quando a dep
**já estava** no `package.json` e os tokens `.dark` já estavam completos. O tema foi ligado como
**opt-in** (padrão continua claro), a dependência deixou de ser morta e a contradição fechou. Ata em
[`docs/DECISOES.md`](DECISOES.md) (2026-07-24 · F19-UX); evidências em `docs/F19-RELATORIO.md`.
...
| **Correções em produção (F13)** | — | 4 defeitos relatados (não são itens deste backlog) | ✅ **CONCLUÍDA em 23/07/2026** pela `docs/prompts/F13-ultracode.md`. B1+B2 eram o mesmo defeito — um `export type { … }` em módulo `'use server'` derrubava **toda** a escrita do app em produção desde 22/07 (ver `docs/RELATORIO-F13.md`). Também: âncoras da ajuda, responsivo das listas/relatório e a senha da tela de definir-senha fora da URL. **Backlog P2 novo** (do responsivo, não endereçado por "sem redesenhar"): alvos de toque < 40px em ordenação/paginação/stepper do wizard/senha de `/relatorios/acesso`; navs roláveis sem affordance de rolagem; um `src/app/global-error.tsx` para cobrir throw no render do layout |
| **Exigem decisão antes** | — | A8, ~~M11~~ (decidido: 30, na F10), T11, ~~T12~~ (decidido: **ligar** o dark mode, na F19-UX) | **Aberto:** A8 e T11 — registrar em DECISOES.md ao decidir |

> **As três ondas de UX fecharam em 22/07/2026** (Onda 1 → F9 · Onda 2 → F10 · Onda 3 → F11) e, em **23/07/2026**, a **F12** fechou os dois itens que ainda pertenciam a outra fase: **M12** (kits) e **I5** (estoque mínimo), os dois da F5. Deste documento sobram apenas a **carga da F6C** (depende de insumo do Johnny) e o que **exige decisão do Johnny** (A8, T11 — o **T12 fechou** em 24/07/2026 com o dark mode ligado pela F19-UX). Não há Onda 4 — ideia nova entra por ordem de serviço nova, não por este documento.

## 6. Não sugerido de propósito (decisões vigentes respeitadas)

Modo *Atualizar* do import (adiado — correção é no sistema, linha a linha); roles/papéis (nível único é decisão de spec); ~~toggle de dark mode (tema claro por design)~~ — **revogado em 24/07/2026 pela F19-UX**: o tema escuro foi ligado como opt-in, com o claro seguindo de padrão; upload do PDF assinado do termo (adiado, F5 item 5.5); qualquer sincronização automática de dados do dia a dia (entrada é 100% manual por regra); qualquer dependência fora da stack fechada — as sugestões acima usam apenas o que já existe no projeto (`ui/command.tsx`, PapaParse, Supabase realtime) ou componentes shadcn via CLI.

...
- Dev: **Supabase CLI**, **@faker-js/faker** (locale pt_BR, só em `scripts/`), **seedrandom**, **Vitest** (componente tem dois graus: render ESTÁTICO por `renderToStaticMarkup`, funções puras e varredura de fonte em `*.test.ts(x)`; interação real em `*.dom.test.tsx`, ver a entrada seguinte), **@testing-library/react** + **/dom** + **/user-event** e **happy-dom** (só devDependencies, só no projeto Vitest `dom`, arquivos `*.dom.test.tsx` — teste de interação de componente; MIT, aprovado pelo Johnny 22/09/2026, reauditoria passo 5), **tsx**, **Playwright** (só em `scripts/`: `design/capturar.mjs` — as fotos de tela do sistema de design —, `design/medir-tabela.mjs` e `design/medir-acessibilidade.mjs` (F44, o estado expandido na árvore de acessibilidade); `smoke-f19.mjs` o resolve por `npx`, sem depender do pacote local. MIT, R$ 0, aprovado pelo Johnny 30/08/2026; ata em `docs/DECISOES.md`), ESLint + Prettier
